...
class ResNetExtractor(ImageFeatureExtractor): 
    ...
    def __init__(self, 
        ...
        # Setup logging
        self.logger = create_logger(job_name="resnet_extractor")
        ...
        input_dim = self.SUPPORTED_MODELS.get(model_name, 512)
        self.use_projection = embed_dim != self.SUPPORTED_MODELS.get(model_name, 512)
    ...
    def preprocess(self, data: Any) -> torch.Tensor:
        """
        Preprocess single image data for ResNet model.
    ...
    def feature_dim(self) -> dict:
        ...
        return  {
            "model_dim": self.SUPPORTED_MODELS.get(self.model, 512), 
    ...
    def __del__(self):
        ...
        if hasattr(self, 'logger'):
            self.logger.info("ResNetExtractor instance destroyed")