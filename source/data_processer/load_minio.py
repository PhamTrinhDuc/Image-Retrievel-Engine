...
logger = create_logger("load_data")
...
def get_images_path() -> list[str]:
    ...
    logger.info(f"Found {len(images_path)} images to upload")
...
def load_to_minio_parallel():
    ...
    logger.info(f"Total images: {len(images_path)}")
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Uploading"):
            ...
            if result[1]:  # Success
                ...
                if success_count % 100 == 0:  # Log every 100 successful uploads
                    logger.info(f"✅ Uploaded {success_count} images successfully")
            else:  # Failed
                fail_count += 1
                logger.info(f"❌ Failed: {result[0]}, Error: {result[2]}")
...
def load_to_minio_batch():
    ...
    logger.info(f"Total images: {len(images_path)}, Batches: {len(batches)}")
